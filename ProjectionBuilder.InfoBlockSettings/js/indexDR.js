            ...
            function()
            {
               ...
               for (var i = 0; i < aCards.length; i++)
               {
                  ...
                  if (aBlock[i] != undefined)
                  {
                     ...
                     $('#'+aBlock[i].ID+'_content').html("");
                     if (aBlock[i].P == 0)
                     {
                        $('#'+aBlock[i].ID+'_content').append("<div id='"+aBlock[i].ID+"_content_header' class='sp_content_header'></div>");
                        $("#"+aBlock[i].ID+"_content_header").css({"width":"auto", "float":"left", "height":"auto", "overflow":"hidden", "border":"1px solid #eaeaea", "text-align":"center"}).html("<strong>"+header_str+"</strong>"+aBlock[i].D);
                        $('#'+aBlock[i].ID+'_content').append(content_str);
                     }
                     else
                     {
                        var br_str = "";
                        ...
                        $('#'+aBlock[i].ID+'_content').append("<div id='"+aBlock[i].ID+"_content_header' class='sp_content_header'></div>");
                        $("#"+aBlock[i].ID+"_content_header").css({"width":"auto", "height":"auto", "overflow":"hidden", "border":"1px solid #eaeaea", "text-align":"center"}).html("<strong>"+header_str+"</strong>"+aBlock[i].D);
                        $('#'+aBlock[i].ID+'_content').append(br_str+content_str);
                     }
                     if (aBlock[i].header.W != 0)
                     {
                        $("#"+aBlock[i].ID+"_content_header").width(aBlock[i].header.W);
                     }
                     if (aBlock[i].header.H != 0)
                     {
                        $("#"+aBlock[i].ID+"_content_header").height(aBlock[i].header.H);
                     }
                     $("#"+aBlock[i].ID+"_content_header").css("line-height", $("#"+aBlock[i].ID+"_content_header").height()+"px");
                     $("#"+aBlock[i].ID+"_content_header").resizable();
                     $("#"+aBlock[i].ID+"_content_header").bind
                     (
                        "resize",
                        {
                           cell:i
                     ...
                     (
                        ...
                        function(ev, ui)
                        {
                           aBlock[ev.data.cell].header.W = $(this).width();
                           aBlock[ev.data.cell].header.H = $(this).height();   
                        }                        
                     );
                     
                     $('#'+aBlock[i].ID+'_content').height($("#"+aBlock[i].ID+"_content").parent().height()-6);   
                  }
               }
            }
         );
   ...
   (
      {
         ...
         drop: function(ev, ui) 
         {
            ...
            droppable_pattern.html("<div id='" + aSP[cur_aSP_cell].ID + "_content' class='SP_CONTENT'><div id='load_" + aSP[cur_aSP_cell].ID + "' class='LOAD'></div></div><a id='close_" + aSP[cur_aSP_cell].ID + "' class='CLOSE_SP' href='#'><img src='img/cross.png' style='float:right; margin:5px;' / ></a>");      
...
function switchBlockSV(block_id)
{
   var cell;
...
{
   $('#LC').append("<div class='SP' id='"+aBlock[array_cell].ID+"' style='position: absolute;'><div id='" + aBlock[array_cell].ID + "_content' class='SP_CONTENT'><div id='load_" + aBlock[array_cell].ID + "' class='LOAD'></div></div><a id='close_" + aBlock[array_cell].ID + "' class='CLOSE_SP' href='#'><img src='img/cross.png' style='float:right; margin:5px;' / ></a></div>");
...
function switchColumnsNumber(number_value)
{
...
function popup()
{
   ...
   $('#popup1').animate({opacity: 1}, 500);
}

function renderPopup(sp_id, cur_arr_cell)
{
      ...
      $('#popup1').append("<a id='default_a' class='event tooltip' href='javascript:void(0)' style='background:url(img/default.png) no-repeat center; width: 16px'><span>Переключение режима просмотра \"по умолчанию - заданной конфигурации\"</span></a>");
   ...
   for (var i = 0; i < aBlock.length; i++)
   {
      $('#'+aBlock[i].ID).resizable("disable");
      $('#'+aBlock[i].ID).css("opacity", "1");
   }
   $('#popup1').mouseleave
   (
      function()
...
function inDialog(block_id, operation_type, cell)
{
