...
var first_loaded = false; // первая загрузка формы настроек (булева переменная)
var counter = 0; // инкрементируемый счётчик для присвоения уникальных id контролам
var cur_KS_id = 0; // ID текущего шаблона
...
function switchEditor(textarea, state) // переключает html редактор на протое поле ввода и наоборот
{            
   if (state == true)
   {               
...
{
   if ($('#'+cur_PTN_id+'.ACC_section_ns').length != 0)
   {
