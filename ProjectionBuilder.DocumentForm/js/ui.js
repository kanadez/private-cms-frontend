...
function applyMode(mode)
{
   ...
   else if (mode == "content_separator_styling")
   {
      if ($('#other_separator_symbol_input').val() != "")
      {
         ...
         if (len == 0)
         {
            var content_str = aBlock[cell].content;
         }
         else
         {
            var content_str = aBlock[cell].content.substr(0, len);
         }
...
function resetStyling() // сбрасывает заданные стили при нажатии Отмена в диалоге
{
   ...
   if (len == 0)
   {
      ...
      {
         var content_str = removeUnderline(aBlock[cell].content);
      }
      else
      {
         var content_str = aBlock[cell].content;
   ...
   else
   {
      if (aBlock[cell].U == false)
      {
         var content_str = removeUnderline(aBlock[cell].content.substr(0, len));
      }
      else
      {
         var content_str = aBlock[cell].content.substr(0, len);
